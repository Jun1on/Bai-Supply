...
contract BaiVaultNFT is ERC721Enumerable {
    string public uri;
...
contract Vault is ReentrancyGuard, BaiVaultNFT {
    IPriceSource public CollateralPriceSource;

    using SafeERC20 for IBai;
    ...
    constructor(
        ...
        address fees_,
        uint _maxPerVault
    ) BaiVaultNFT(name, symbol, baseURI) {
    ...
    function getDebtCeiling() public view returns (uint256) {
        return bai.balanceOf(address(this));
    }
        ...
        onlyVaultOwner(vaultID)
    {
        ...
        bai.safeTransfer(msg.sender, amount);
    ...
    function payBackToken(uint256 vaultID, uint256 amount) external {
        require(bai.balanceOf(msg.sender) >= amount, "Token balance too low");
        ...
        bai.safeTransferFrom(msg.sender, address(this), amount);
    ...
    function liquidateVault(uint256 vaultID) external {
        ...
        require(
            bai.balanceOf(msg.sender) >= debtValue,
        ...
        bai.safeTransferFrom(msg.sender, address(this), debtValue);
    ...
    returns(
        ...
        _debtAsset = address(bai);
        _vault = address(this);
    }
}

contract BaiVault is Vault, Ownable {
    constructor(
        address CollateralPriceSourceAddress,
        uint256 minimumCollateralPercentage,
        string memory name,
    ...
    function recover(uint256 amountToken, address _address) public onlyOwner {
        bai.transfer(_address, amountToken);
    }
